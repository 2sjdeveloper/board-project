...
<template>
  <div>
    ...
    <BoardList v-if="listView" v-bind:list="boardList" @show-write="showWrite" @board-delete="deleteBoard"
      @show-read="showRead" />
    <!-- boardList에 담겨있는 값을 list라는 attribute에 담아주고 BoardList라는 component에서 props 속성을 이용해 데이터를 넘겨줄 수 있음 -->
    <BoardWrite v-if="writeView" v-on:save-board="saveBoard" />
    <BoardRead v-if="readView" v-bind:obj="board" @show-list="showList"></BoardRead>
    <!-- html 전달-->
    <FooterComponent v-vind:data="htmlData">
      <template v-slot:footer><p>Since 2020</p></template>
      <template v-slot:header><h3>header에 들어갈 내용</h3></template>
      <template v-slot:default><p>헬로우 월드</p></template>
    </FooterComponent>
...
export default {
  data() {
    return {
      ...
      boardList: [
        ...
        { no: 3, title: "오늘 점심은 서브웨이", content: "서브웨이 짱!", view: 1 },
      ],
      htmlData: '<p>하이</p>'
    }
  },
  ...
  methods: {
    ...
    showRead(board) { //상세화면 보여주기
      ...
      let idx = -1; //해당 위치의 값을 새로운 값으로 변경하기 위해서. 임의의 초기값 -1
      let viewCnt = 0;
    ...
    saveBoard(title, content) { //신규글 등록
      ...
      let no = parseInt(this.boardList[idx].no) + 1; //새로운 데이터의 no값을 지정.
